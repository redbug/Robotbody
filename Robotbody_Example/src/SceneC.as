package 
{
	...
	public class SceneC extends BaseScene
	{

		private const MY_SWF		:Array = ["preloader_C.swf", "swf_C.swf"];
		private const MY_TXT		:Array = ["text_C.txt"];
		private const MY_MUSIC		:Array = ["music_C.mp3"];
		private const MY_XML		:Array = ["shortcut_C.xml"];
        ...
        private function progressHandler( itemName:String, percentLoaded_item:String, percentLoaded_total:String  ):void
        {
            ...
            if( resManager.get("preloader_C.swf") != null && !isPreloaderLoaded )
            {
                isPreloaderLoaded = true;
                var preloader_swf:MovieClip = resManager.get( "preloader_C.swf" ) as MovieClip;
		...
		override public function accessSWF():void
		{
            ...
            var swf_C_swf  :MovieClip = resManager.get("swf_C.swf") as MovieClip;
            var symbol_mc  :MovieClip = swf_C_swf.getChildByName("symbol_mc") as MovieClip;
            addToContextView( symbol_mc );
            Toolkits.playFromAToEND( symbol_mc, 0);

            shortcut.key_txt = swf_C_swf.getChildByName("key_txt") as TextField;
            shortcut.func_txt = swf_C_swf.getChildByName("func_txt") as TextField;
            var title_txt:TextField = swf_C_swf.getChildByName("title_txt") as TextField;
            addToContextView( title_txt ); 
            
            text_txt = swf_C_swf.getChildByName("source_txt") as TextField;
		}	
		
		override public function accessTXT():void
		{
            var myPattern	:RegExp = /[\r]/g;
            text_txt.text = String( resManager.get("text_C.txt") ).replace( myPattern, "");
		...
		{
            var music:Sound	= resManager.get("music_C.mp3") as Sound;
		...
		{
            var xml:XML = resManager.get("shortcut_C.xml") as XML;
